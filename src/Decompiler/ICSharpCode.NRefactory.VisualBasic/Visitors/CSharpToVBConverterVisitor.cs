...
using ICSharpCode.NRefactory.VisualBasic.Ast;

namespace ICSharpCode.NRefactory.VisualBasic.Visitors
{
	...
	/// </summary>
	public class CSharpToVBConverterVisitor : CSharp.IAstVisitor<object, VisualBasic.AstNode>
	{
		...
		{
			var attr = new VisualBasic.Ast.Attribute();
...
//					CreateImplementsClausesForMethod(result);
		...
		void ConvertNodes<T>(IEnumerable<CSharp.AstNode> nodes, VisualBasic.AstNodeCollection<T> result, Func<T, T> transform = null) where T: VisualBasic.AstNode
		{
		...
		void ConvertMembers<T, S, M>(CSharp.AstNode parent, T result, Role<S> sourceRole, Role<M> targetRole) where T : VisualBasic.AstNode where S : CSharp.AstNode where M : VisualBasic.AstNode
		{
		...
		T EndNode<T>(CSharp.AstNode node, T result) where T : VisualBasic.AstNode
		{
			if (result != null) {
		...
		void CopyAnnotations<T>(CSharp.AstNode node, T result) where T : VisualBasic.AstNode
		{
